...
| **Local Device Vault**      | **`expo-secure-store` / Client Vault**                                  | Multi-platform in-device vault: hardware-backed Keychain/Keystore via `expo-secure-store` on iOS/Android, origin-isolated storage on Web; credentials are forwarded in-memory per prompt with zero cloud relay retention.                                      |
