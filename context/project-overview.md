...
3. **Open-Source Universal Remote Experience:**
   ...
   - Dual host interfaces: Headless Terminal CLI (`apps/cli`) + Native VS Code Extension (`apps/vscode-extension`), both wrapping `@agent-remote/bridge-core`.
...
- **VS Code Extension Host (`apps/vscode-extension`):** Lightweight extension wrapping `bridge-core` with Status Bar PIN display (`$(radio-tower) Remote: 834-192`) and native modal approvals.
...
- **Web Landing Page & Pairing Shell (`apps/web`):** Next.js / Web landing page with dark developer aesthetics (`#090d16`), interactive terminal emulator, and browser-based remote control `/pair` client.
...
- **Production Deployment:** Multi-stage Docker for Relay (Fly.io/Railway), Vercel hosting for Web Landing, EAS build profiles for Mobile, and `tsup` standalone binary bundling for `npx @agent-remote/cli`.
