...
export function createRelayServer(options: RelayServerOptions = {}): RelayServerInstance {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on(SOCKET_EVENTS.REGISTER_HOST, (data: unknown) => {
      ...
      socket.join(pin);
      console.log(`[Relay] 💻 Host connected & registered room PIN: ${pin.slice(0, 3)}-${pin.slice(3)} (Host: ${hostName})`);
    });
    ...
    socket.on(SOCKET_EVENTS.JOIN_SESSION, (data: unknown) => {
      ...
      socket.join(pin);
      console.log(`[Relay] 📱 Mobile/Web client paired to room PIN: ${pin.slice(0, 3)}-${pin.slice(3)} (${clientName})`);
